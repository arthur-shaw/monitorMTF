...
#' @importFrom rlang data_sym global_env
#'
#' @noRd
...
create_dk_attrib_obj <- function(
  ...
  attrib_df <- susoreview::create_attribute(
    df = df,
    condition = !!rlang::data_sym(var_name) == dk_val,
...
create_bool_attrib_obj <- function(
  ...
  attrib_df <- susoreview::create_attribute(
    df = df,
    condition = !!rlang::data_sym(var_name) == 1,
    attrib_name = glue::glue("{attrib_name}_na"),
...
create_bool_attrib_obj_w_2_vars <- function(
  ...
  attrib_df <- susoreview::create_attribute(
    df = df,
    condition = (
      !!rlang::data_sym(var1_name) == 1 &
      !!rlang::data_sym(var2_name) == 0
    ),
...
hhold_c_dir <- fs::path(info$proj_dir, "01_data", "01_hhold", "02_combined")
...
main_solar_system <- hholds |>
  ...
  dplyr::select(
    interview__id, SEC_C_Q141,
    # attributes of main system
    # hours of electricity
    SEC_C_Q148, SEC_C_Q149
  ) |>
  dplyr::left_join(solar_systems, by = "interview__id") |>
  dplyr::filter(Solar_C_Device__id == SEC_C_Q141) |>
...
key_solar_capacity_qs <- tibble::tribble(
  ...
  # charge phone
  "solar_systems",  "SEC_C_Q122", "solar_charge_phone",
  # power light bulb
  "solar_systems",  "SEC_C_Q123", "solar_charge_lightbulb",
  # run radio
  "solar_systems",  "SEC_C_Q124", "solar_charge_radio",
)
...
attrib_solar_other_na <- solar_systems |>
  dplyr::mutate(
    all_na = dplyr::if_all(
...
hrs_per_day <- tibble::tribble(
  ...
  "solar_hrs_day",        "SEC_C_Q138", "main_solar_system"
)
...
critical_damage <- tibble::tribble(
  ~ var_name,   ~ attrib_name,
  "SEC_C_Q35",  "damage_equip_grid",
  "SEC_C_Q73",  "damage_equip_mini_grid",
  "SEC_C_Q95",  "damage_equip_mini_grid"
) |>
dplyr::mutate(df_name = "hholds")
...
death_injury <- tibble::tribble(
  ~ var_name,   ~ attrib_name,
  "SEC_C_Q162", "injury_main",
  "SEC_C_Q38", "injury_grid",
  "SEC_C_Q76", "injury_mini_grid",
  "SEC_C_Q98", "injury_generator",
  "SEC_C_Q116", "injury_batteries"
)
...
create_na_attrib_obj(
  df_name = "main_stove",
  attrib_name = "stove_type_na",
  var_name = "SEC_G_Q05"
)
...
create_na_attrib_obj(
  df = "main_stove",
  attrib_name = "time_in_kitchen_na",
  attrib_vars = "SEC_G_Q02"
)

# avg time per meal
create_na_attrib_obj(
  df = "main_stove",
  attrib_name = "time_per_meal_na",
  attrib_vars = "SEC_G_Q018"
)

# days used stove in last week
create_na_attrib_obj(
  df = "main_stove",
  attrib_name = "days_use_stove_na",
  attrib_vars = "SEC_G_Q19"
)

# time per day to boil water
create_na_attrib_obj(
  df = "main_stove",
  attrib_name = "min_boil_na",
  attrib_vars = "SEC_G_Q21"
)
...
time_cook <- tibble::tribble(
  ...
  "main_stove", "SEC_G_Q22a", "min_cook_aftn_na",
  "main_stove", "SEC_G_Q22a", "min_cook_even_na",
)
...
attrib_stove_used <- purrr::pmap(
  ...
  .f = ~ susoreview::create_attribute(
    ...
    condition = !!rlang::parse_quo(
      x = glue::glue("SEC_G_Q05 == {..1} & SEC_G_Q13 %in% c(1:12, 111)"),
...
attrib_is_backup_solar <- susoreview::create_attribute(
  ...
  condition = (
    ...
    # for mini-grid
    (SEC_C_Q69 == c(2:4) | SEC_C_Q70 == c(2:4))
  ),
