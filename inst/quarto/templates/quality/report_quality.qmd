...
#'
#' @description Compute and plot density of a variable by
#' - Replacing DK values with missing, to avoid skewing distribution
#' - Computing the full-sample data range that that the plot should cover
#' - Filtering to observations in the target group
#' - Producing either a violin plot or an empty plot, depending on the number
#' of non-NA values found in the group
#'
#' @param df Data frame.
#' @param var Bare name of target variable in the data frame.
#' @param group_var Bare name of group variable.
#' @param group_name Character. Name of group for which produce a plot.
#' @param fill_color Character. Valid name or hex code for ggplot's fill color
#' attribute.
#'
#' @return ggplot2 plot
#'
#' @importFrom dplyr mutate if_else pull filter
#' @importFrom rlang enquo
#' @import ggplot2
plot_group_density <- function(
  df,
  var,
  group_var,
  group_name,
  ...
  df_mod <- df |>
    dplyr::mutate(
      !!rlang::enquo(var) := dplyr::if_else(
        condition = {{var}} == dk_val,
        true = NA,
        false = {{var}}
      )
    )

  # compute range of variable
  full_range <- df_mod |>
    dplyr::pull({{var}}) |>
    range(na.rm = TRUE)
  
  # filter to the group of interest
  df_filtered <- df_mod |>
	  dplyr::filter({{group_var}} == group_name)
  
  # use all observations for "overall" group
  if (group_name == "Overall") {
    df_filtered <- df_mod |>
      dplyr::mutate(!!rlang::enquo(group_var) := "Overall")
  }

  # compute range of group
  filtered_range <- dplyr::pull(df_filtered, {{var}})

  # get number of non-NA values
  n_non_na <- filtered_range[!is.na(filtered_range)] |>
    length()

  # if more than 5 non-NA obs, plot density
  # otherwise, return a blank plot
  if (n_non_na > 5) {

    plot <- df_filtered |>
      ggplot2::ggplot(ggplot2::aes(x = {{var}}, y = {{group_var}})) +
      ...
      # set coordinates to be full range of data
      # not just range for the group
      ggplot2::coord_cartesian(xlim = full_range)
...
```{r}
#| label: response rate
response_rate <- supervisor_interviewer_interview |>
  dplyr::left_join(
    y = dfs_raw[[1]],
    by = c("interview__id", "interview__key")
  ) |>
	dplyr::mutate(
    n_contacted = TRUE,
    n_found = SEC_Cov_Q01 == 1,
    n_eligible_resp = SEC_Cov_Q03 == 1,
    n_consent = SEC_Cov_Q05 == 1
  ) |>
	...
	dplyr::mutate(
    dplyr::across(
      .cols = c(n_found, n_eligible_resp, n_consent),
      .fns = ~ .x / n_contacted,
      .names = "pct_{.col}"
    )
  ) |>
	dplyr::rename(
    pct_found = pct_n_found,
    pct_eligible_resp = pct_n_eligible_resp,
    pct_consent = pct_n_consent
  ) |>
	dplyr::select(supervisor, n_contacted, pct_found, pct_eligible_resp, pct_consent)

response_rate_table <- response_rate |>
	gt::gt(rowname_col = "supervisor") |>
	gt::tab_header(title = "Response rate") |>
	gt::cols_label(
    n_contacted = gt::html("Contacted<br>(N)"),
    pct_found = gt::html("Located<br>(%)"),
    pct_eligible_resp = gt::html("Got respondent<br>(%)"),
    pct_consent = gt::html("Got consent<br>(%)")
  ) |>
  # format percentages without including % marker
	gt::fmt_number(
...
solar_oth_device_stats <- solar_oth_device_overall |>
	dplyr::bind_rows(solar_oth_device_by_team) |>
...
coding_stats <- coding_overall |>
  ...
  dplyr::mutate(
    ...
    n_obs,
    # day
    pct_bottom_day, pct_top_day,
    # night
    pct_bottom_night, pct_top_night,
  )

avail_top_bottom_table <- coding_stats |>
  # add columns for distributions
  dplyr::mutate(
    distrib_day = source,
    distrib_night = source
  ) |>
  gt::gt(rowname_col = "source") |>
  gt::tab_header(title = "Percent of hours of availability top- or bottom-coded, by team") |>
	gt::cols_label(
    n_obs = gt::html("Obs.<br>(N)"),
    pct_bottom_day = gt::html("Bottom<br>(%)"),
    pct_top_day = gt::html("Top<br>(%)"),
    distrib_day = "Distribution",
    pct_bottom_night = gt::html("Bottom<br>(%)"),
    pct_top_night = gt::html("Top<br>(%)"),
    distrib_night = "Distribution"
  ) |>
	gt::tab_spanner(
    label = "Day",
    columns = c(pct_bottom_day, pct_top_day, distrib_day)
  ) |>
	gt::tab_spanner(
    label = "Night",
    columns = c(pct_bottom_night, pct_top_night, distrib_night)
  ) |>
  gt::fmt_percent(
    columns = dplyr::starts_with("pct_"),
    decimals = 1,
	...
	gt::sub_missing() |>
	gt::tab_options(
    heading.background.color = "#6f3996",
    column_labels.background.color = "#6f3996"
  ) |>
  # distribution of daytime values
	gt::text_transform(
    locations = gt::cells_body(columns = c(distrib_day)),
    fn = function(x) {
      # produce a density plot by group whose name
      # appears in the `group` column
      purrr::map(
        .x = x, 
        .f = ~ plot_group_density(
          df = coding_dfs,
          var = val_day,
          group_var = source,
          group_name = .x,
      ...
      # embed the plot as an image in the gt table
      gt::ggplot_image(
        height = gt::px(50),
        aspect_ratio = 3
      )
	...
	gt::text_transform(
    locations = gt::cells_body(columns = c(distrib_night)),
    fn = function(x) {
      # produce a density plot by group whose name
      # appears in the `group` column
      purrr::map(
...
dk_blackout <- dk_blackout_grid |>
  ...
  dplyr::distinct(
    supervisor, interview__id, interview__key, source, .keep_all = TRUE
  ) |>
	tidyr::pivot_wider(
...
expenditure_items_counts <- supervisor_interviewer_interview |>
  ...
  dplyr::mutate(
    ...
    dplyr::across(
      ...
      .fns = ~ dplyr::case_when(
        .x == 1 ~ TRUE,
        .x != 1 & !is.na(interview__id) ~ FALSE,
        .x != 1 & is.na(interview__id) ~ NA,
      )
    ),
    ...
    n_food_items = dplyr::if_else(
      condition = is.na(interview__id),
      true = NA_integer_,
      false = n_food_items
    ),
    dplyr::across(
      .cols = dplyr::matches("SEC_O_(One|Q12)_Month"),
      .fns = ~ dplyr::case_when(
        .x == 1 ~ TRUE,
        .x != 1 & !is.na(interview__id) ~ FALSE,
        .x != 1 & is.na(interview__id) ~ NA,
    ...
    n_non_food_items = rowSums(
      dplyr::pick(dplyr::matches("SEC_O_(One|Q12)_Month")),
      na.rm = TRUE
    ),
    n_non_food_items = dplyr::if_else(
      condition = is.na(interview__id),
      true = NA_integer_,
      false = n_non_food_items
    ),
    n_items = rowSums(
      dplyr::pick(n_food_items, n_non_food_items),
      na.rm = TRUE
    ),
    n_items_series = n_items,
    no_food_items = dplyr::if_else(
      condition = !is.na(interview__id),
      true = (n_food_items == 0),
      false = NA_integer_
    ),
    no_non_food_items = dplyr::if_else(
      condition = !is.na(interview__id),
      true = (n_non_food_items == 0),
      false = NA_integer_
    ),
    no_items = dplyr::if_else(
      condition = !is.na(interview__id),
      true = (n_items == 0),
      false = NA_integer_
    )
...
expenditure_item_stats <- expenditure_items_overall |>
	dplyr::bind_rows(expenditure_items_by_team) |>
	dplyr::mutate(
    dplyr::across(
...
```{r}
#| label: time spent preparing main stove, prevalence of DK and distribution

time_spent_prep_stove <- supervisor_interviewer_interview |>
  dplyr::left_join(
    y = main_stove,
    by = c("interview__id", "interview__key")
  ) |>
  dplyr::mutate(
    n_obs = !is.na(SEC_G_Q18),
    is_dk = (SEC_G_Q18 == -888),
    SEC_G_Q18 = dplyr::if_else(
      condition = is_dk == TRUE,
      true = NA,
      false = SEC_G_Q18
    )
  ) |>
	dplyr::select(supervisor, interview__id, n_obs, is_dk, SEC_G_Q18)

time_spent_prep_stove_by_team <- time_spent_prep_stove |>
  dplyr::group_by(supervisor) |>
	dplyr::summarize(
    dplyr::across(
      .cols = c(n_obs, is_dk),
    ...
    min = min(SEC_G_Q18, na.rm = TRUE),
    max = max(SEC_G_Q18, na.rm = TRUE),
    med = median(SEC_G_Q18, na.rm = TRUE)
  ) |>
	dplyr::ungroup() |>
	dplyr::select(supervisor, n_obs, is_dk, min, med, max)

time_spent_prep_stove_overall <- time_spent_prep_stove |>
	dplyr::summarize(
    dplyr::across(
      .cols = c(n_obs, is_dk),
      .fns = ~ sum(.x, na.rm = TRUE)
...
time_spent_prep_stove_stats <- time_spent_prep_stove_overall |>
  dplyr::bind_rows(time_spent_prep_stove_by_team) |>
  compute_pct(num = is_dk, denom = n_obs) |>
  dplyr::rename(pct_dk = is_dk) |>
	dplyr::mutate(
    dplyr::across(
      .cols = c(min, med, max),
      .fns = ~ dplyr::if_else(
        condition = is.infinite(.x),
        true = NA,
        false = .x
      )
    ),
	...
	dplyr::select(supervisor, n_obs, pct_dk, min, med, max, group)

time_spent_prep_stove_table <- time_spent_prep_stove_stats |>
	gt::gt(rowname_col = "supervisor") |>
	gt::tab_header(title = "Time spent, by team") |>
  gt::cols_label(
    n_obs = gt::html("Obs<br>(N)"),
    pct_dk = gt::html("DK<br>(%)"),
    min = "Min.",
    med = "Med.",
    max = "Max.",
    group = "Distribution"
...
```{r}
#| label: prepare time collecting data

time_collecting_fuel <- supervisor_interviewer_interview |>
	dplyr::left_join(dfs_filtered$time_collecting, by = c("interview__id", "interview__key")) |>
  dplyr::mutate(
    n_obs = !is.na(time_collecting__id),
    is_dk_get = SEC_I_Q12 == -888,
    is_dk_prep = SEC_I_Q14 == -888
  ) |>
	dplyr::select(
    supervisor, interview__id, n_obs, is_dk_get, is_dk_prep,
    get = SEC_I_Q12, prep = SEC_I_Q14
  )

time_collecting_by_team <- time_collecting_fuel |>
  dplyr::group_by(supervisor) |>
	dplyr::summarize(
    dplyr::across(
      .cols = c(n_obs, is_dk_get, is_dk_prep),
      .fns = ~ sum(.x, na.rm = TRUE)
    ),
    dplyr::across(
      .cols = c(get, prep),
	...
	dplyr::select(
    supervisor, n_obs,
    dplyr::starts_with("is_dk"),
    dplyr::starts_with("min"),
    dplyr::starts_with("med"),
    dplyr::starts_with("max")
  )

time_collecting_overall <- time_collecting_fuel |>
	dplyr::summarize(
    dplyr::across(
      .cols = c(n_obs, is_dk_get, is_dk_prep),
      .fns = ~ sum(.x, na.rm = TRUE)
...
time_collecting_stats <- time_collecting_overall |>
	dplyr::bind_rows(time_collecting_by_team) |>
  compute_pct(num = is_dk_get, denom = n_obs) |>
  compute_pct(num = is_dk_prep, denom = n_obs) |>
  dplyr::rename(
    pct_dk_get = is_dk_get,
    pct_dk_prep = is_dk_prep
  ) |>
	dplyr::mutate(
    dplyr::across(
      .cols = c(
    ...
    distrib_get = supervisor,
    distrib_prep = supervisor
  ) |>
  dplyr::select(
    supervisor, n_obs,
    pct_dk_get, min_get, med_get, max_get, distrib_get,
    pct_dk_prep, min_prep, med_prep, max_prep, distrib_prep
  )

time_collecting_table <- 	time_collecting_stats |>
	gt::gt(rowname_col = "supervisor") |>
	gt::tab_header(title = "Time spent gathering fuel, by team") |>
  gt::cols_label(
    n_obs = gt::html("Obs<br>(N)"),
    pct_dk_get = gt::html("DK<br>(%)"),
    min_get = "Min.",
    med_get = "Med.",
    max_get = "Max.",
    distrib_get = "Distribution",
    pct_dk_prep = gt::html("DK<br>(%)"),
    min_prep = "Min.",
    med_prep = "Med.",
    max_prep = "Max.",
    distrib_prep = "Distribution"
  ) |>
  gt::fmt_integer(columns = n_obs) |>
  gt::fmt_percent(
    columns = dplyr::starts_with("pct_"),
  ...
  # distribution of "get"
	gt::text_transform(
    locations = gt::cells_body(columns = c(distrib_get)),
    fn = function(x) {
      # produce a density plot by group whose name
      # appears in the `group` column
      purrr::map(
  ...
  # distribution of "prep"
	gt::text_transform(
    locations = gt::cells_body(columns = c(distrib_prep)),
    fn = function(x) {
      # produce a density plot by group whose name
      # appears in the `group` column
      purrr::map(
	...
	gt::tab_spanner(
    label = "Acquisition",
    columns = dplyr::ends_with("_get")
  ) |>
	gt::tab_spanner(
    label = "Preparation",
    columns = dplyr::ends_with("_prep")
  )
	

time_collecting_table
```
